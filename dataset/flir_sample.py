...
class FLIRSampleGetter(BaseSampleGetter):
    '''FLIR dataset
    ...
    def image(img_path: str):
        ...
        return np.concatenate([image, the_img[..., None]], axis=-1)
    ...
    def set_train_augment(self, augment_cfg: CN, input_size, img_path_sampler):
        ...
        self.compose_augment = augment.Compose([
            ...
            augment.GenRadar(),
            augment.Normalize(mean=[0.485, 0.456, 0.406, 0.456, 0.456], std=[0.229, 0.224, 0.225, 0.224, 0.224]),
            augment.ToTensor('cpu'),
        ])
        return self

    def set_eval_augment(self, input_size):
        self.eval_augment = eval_augment_flir(input_size, 'cpu')
        return self
...
def eval_augment_flir(input_size, device):
...
recover_bboxes_prediction_flir = simple_recover_bboxes_prediction
