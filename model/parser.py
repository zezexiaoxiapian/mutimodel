...
def _get_act_layer(act, quant=False):
    if act == 'linear':
        return None
    if quant:
        return nn.ReLU()
...
class Upsample(nn.Module):
    def __init__(self, scale_factor: float=1., mode: str='nearest'):
        super().__init__()
...
class FC(nn.Module):
    def __init__(self, input: int, output: int, activation: str):
        ...
        self.act = _get_act_layer(activation)
        self.input = input
...
def build_grid(height, width) -> torch.Tensor:
    shiftx = torch.arange(0, height)
    shifty = torch.arange(0, width)
    ...
    return xy_grid.float()

class Decode(nn.Module):
    def __init__(self, num_classes: int, stride: int, anchors, onnx: bool=False):
    ...
    def respawn_grid(self, height, width, device):
        self.xy_grid = build_grid(height, width).to(device)
    ...
    def forward(self, conv):
        ...
        anchor_per_scale = conv_shape[3] // (5 + self.num_classes)

        conv = conv.view(batch_size, out_size_h, out_size_w,
            anchor_per_scale, 5 + self.num_classes)
        conv_raw_dxcdyc, conv_raw_dwdh, conv_raw_logit = torch.split(
            conv, [2, 2, 1 + self.num_classes], dim=-1)

        if self.onnx:
            xy_grid = build_grid(out_size_h, out_size_w).to(conv.device)
        else:
        ...
        # decode xy
        pred_xcyc = (xy_grid + 2 * torch.sigmoid(conv_raw_dxcdyc) - 0.5) * self.stride
        pred_wh = (2 * torch.sigmoid(conv_raw_dwdh)) ** 2 * self.anchors
        pred_xymin = pred_xcyc - pred_wh / 2
        pred_xymax = pred_xcyc + pred_wh / 2
        # re-concat items
        pred_bbox = torch.cat((pred_xymin, pred_xymax, conv_raw_logit), -1)
        return pred_bbox
...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, opt: dict, onnx: bool=False):
        super().__init__()
        self.decode = Decode(opt['classes'], opt['stride'], opt['anchors'], onnx)
        self.opt = opt
    ...
    def torch_layers(self, quant: bool=False, onnx: bool=False) -> List[nn.Module]:
        ...
        for l in self.gen_layers():
            ...
            if name == 'convolutional':
                ...
                act = _get_act_layer(l['activation'], quant)
                if act is not None:
                ...
                blocks = Upsample(scale_factor=l['stride'], mode='nearest')
                ...
                opt['anchors'] = np.array(opt['anchors'], dtype=np.float32).reshape(-1, 2)
                ...
                blocks = nn.Sequential()
                blocks.add_module('bn', nn.BatchNorm2d(layer_info.channels))
                act = _get_act_layer(l['activation'], quant)
                if act is not None:
                    blocks.add_module('act', act)
            else:
